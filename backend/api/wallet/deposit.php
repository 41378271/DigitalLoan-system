...
try{
  ...
  wallet_add_tx($conn, $user_id, "deposit", $amount, $newBal, "manual_deposit");

  $conn->commit();
  json_out(["success"=>true, "message"=>"Deposit successful", "balance"=>$newBal, "currency"=>$w["currency"]]);
}catch(Exception $e){
  $conn->rollback();
  json_out(["success"=>false, "message"=>"Deposit failed"]);
}